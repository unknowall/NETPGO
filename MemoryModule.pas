...
function NT_IMAGE_FIRST_SECTION( const ntheader: PIMAGE_NT_HEADERS ): PImageSectionHeader;
begin
...
begin
  ...
  Section := PIMAGE_SECTION_HEADER(NT_IMAGE_FIRST_SECTION(Module.Headers)); //{$IFNDEF FPC}^{$ENDIF}
...
var
  ...
  begin
    Result := Pointer(UInt64(OldPtr) - Module.Headers.OptionalHeader.ImageBase + UInt64(CodeBase));
  end;
...
begin
  ...
  try
    ...
    locationdelta := UInt64(code) - old_header.OptionalHeader.ImageBase;
